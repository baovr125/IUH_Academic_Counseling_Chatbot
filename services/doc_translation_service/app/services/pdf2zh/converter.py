...
class TranslateConverter(PDFConverterEx):
    def __init__(
        ...
        self.noto = noto
        self.translator: OllamaPDFTranslator = None
        # param e.g. "ollama_pdf:qwen2.5:14b"
        ...
        for id, new in enumerate(news):
            ...
            while size_scale >= 0.5:
                ...
                while ptr < len(new):
                    ...
                    if not vy_regex and ch == ' ':
                        word_adv = 0
                        next_ptr = ptr
                        while next_ptr < len(new) and new[next_ptr] != ' ' and not new[next_ptr:].startswith('{'):
                            next_ch = new[next_ptr]
                            next_fcur_ = None
                            try:
                                if next_fcur_ is None and self.fontmap["tiro"].to_unichr(ord(next_ch)) == next_ch:
                                    next_fcur_ = "tiro"
                            except Exception:
                                pass
                            if next_fcur_ is None:
                                next_fcur_ = self.noto_name
                            if next_fcur_ == self.noto_name:
                                word_adv += self.noto.char_lengths(next_ch, size)[0]
                            else:
                                word_adv += self.fontmap[next_fcur_].char_width(ord(next_ch)) * size
                            next_ptr += 1
                        
                        if x + adv + word_adv > max_x_allowed:
                            if cstk:
                                ops_vals.append({
                                    "type": OpType.TEXT,
                                    "font": fcur,
                    ...
                    if (                                # 输出文字缓冲区
                        fcur_ != fcur                   # 1. 字体更新
                        or vy_regex                     # 2. 插入公式
                        or x + adv > max_x_allowed      # 3. 到达右边界（可能一整行都被符号化，这里需要考虑浮点误差）
                    ):
                        if cstk:
                            ops_vals.append({
                                "type": OpType.TEXT,
                    ...
                    # Ép xuống dòng nếu vượt biên ngang (Bỏ điều kiện "if brk")
                    if x + adv > max_x_allowed:
                        x = x0
                        lidx += 1
                        tx = x
                    if vy_regex:  # 插入公式
