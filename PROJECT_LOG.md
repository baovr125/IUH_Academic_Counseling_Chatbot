...
| **25** | Hoàn thành Phase 1 (Roadmap) - Metrics & Feedback | Thêm theo dõi độ trễ (`latency_ms`) và số lượng token (prompt/completion) vào DB. Bổ sung API và giao diện đánh giá phản hồi (Like/Dislike/Comment) tại `backend/app/routers/chat.py` và `ChatMessageBubble.tsx`. Triển khai Context Sandbox chống Indirect Prompt Injection trong `backend/app/services/rag_service.py`. |
| **26** | Hoàn thành Phase 2 (Roadmap) - Retrieval Quality | Áp dụng kỹ thuật mở rộng ngữ cảnh (Neighbor Chunk Expansion) cho Retrieval, tạo Cache TTL bằng `cachetools` cho query embeddings (giảm tải 200-500ms), thêm logic cào dữ liệu tăng dần bằng `content_hash` trong crawler, và bổ sung ngưỡng Confidence Threshold (0.15) với fallback "Không biết" khi độ tin cậy thấp. |
