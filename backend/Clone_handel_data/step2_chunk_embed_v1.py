...
INPUT_DIR = r"G:\Khoa_Luan\Source_code\data\markdown_crawl5\markdown_updates" 
FILE_EXTENSION = ".md"
...
def process_single_markdown(file_path):
    ...
    try:
        ...
        # ========================================================
        # [BƯỚC BỔ SUNG QUAN TRỌNG]: XÓA DỮ LIỆU CŨ TRƯỚC KHI CẬP NHẬT
        # ========================================================
        try:
            # Tìm và xóa toàn bộ các chunk trong DB có metadata 'source' khớp với tên file hiện tại
            collection.delete(where={"source": filename})
            # print(f"🧹 Đã dọn dẹp phiên bản cũ của {filename} trong ChromaDB (nếu có).")
        except Exception as e:
            # Bỏ qua nếu có lỗi nhẹ (ví dụ: DB mới tạo chưa có dữ liệu nào)
            pass
        # ========================================================
        ...
        # Tiến hành nạp vào DB (Lúc này upsert sẽ đóng vai trò như insert hoàn toàn mới)
        if ids:
            collection.upsert(
                ids=ids,
                embeddings=embeddings,
