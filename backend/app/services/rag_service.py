...
async def build_rag_payload(session_id: str, content: str):
    ...
    Note: `content` should already be normalized by the caller — no double normalization here.
    """
    history = await asyncio.to_thread(get_session_history_from_db, session_id)

    # Exclude current user message if already persisted (streaming mode) to avoid duplicates
    filtered_history = [
    ...
    # Convert past turns to Gemini content objects (use filtered history to avoid duplicate)
    contents = []
